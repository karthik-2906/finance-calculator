(function () {
    class fdCalculator {
        ...
        updateAllResults() {
            const startInvestment = parseInt(this.investment.textContent.replace(/[₹,]/g, '')) || 0;
            const endInvestment = Number(this.investmentInputBox.value) || 0;
            // this.animateValue(this.investment, startInvestment, endInvestment, 100);
            this.investment.textContent = `₹ ${endInvestment.toLocaleString('en-IN')}`;

            const interest = this.calculateInterest();
            const startReturns = parseInt(this.returns.textContent.replace(/[₹,]/g, '')) || 0;
            // this.animateValue(this.returns, startReturns, interest, 100);
            this.returns.textContent = `₹ ${interest.toLocaleString('en-IN')}`;

            const totalAmount = interest + endInvestment;
            const startTotal = parseInt(this.totalAmount.textContent.replace(/[₹,]/g, '')) || 0;
            // this.animateValue(this.totalAmount, startTotal, totalAmount, 100);
        ...
        updateTimeDropdown(durationType) {
            const maxValues = { Years: 25, Months: 11, Days: 31 };
